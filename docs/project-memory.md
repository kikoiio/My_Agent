...
### 当前已完成（Post-P6 全量）
- ✅ STT：`faster-whisper` base 模型，中文，`_stt_stage` + `_transcribe()` 真实实现
- ✅ TTS：`edge-tts`（免费微软 TTS），`backend/tts/edge_tts_client.py`，`play_audio_mp3()` 播放
- ✅ 唤醒词：Whisper tiny 滑窗关键词检测（2s 窗口 + 50% 重叠），无需自定义模型
- ✅ 人脸识别：insightface buffalo_l，cosine similarity 验证，嵌入向量存 `.npy`
- ✅ 声纹验证：resemblyzer GE2E 模型，平均嵌入 + cosine similarity
- ✅ enroll_owner.py：cv2 摄像头采集 + sounddevice 录音，真实实现
- ✅ 语音主循环：`python main.py --voice`，唤醒→STT→LLM→TTS→播放
- ✅ CI/CD：`.github/workflows/test.yml`，Python 3.11 + 3.12
- ✅ 281 smoke tests 全绿（+24 新增 hardware-mocked 测试）
...
### 2026-05-10 (晚): P7.1–P7.3 实机准备完成

**目标**：硬件到位后跑通 P7.1（硬件检测）+ P7.2（依赖安装）+ P7.3（owner 注册），剩下端到端测试留下一 session。

**P7.1 硬件检测**：
- USB Logitech C922 摄像头识别为 sounddevice [1] (MME) / [14] (WASAPI) / [28]/[29] (WDM-KS)
- USB Philips SPA3809 音箱默认输出 [4] MME，蓝牙音箱也配对了（btha2dp [25/27]）但不是默认
- ACER 内置摄像头作为 cv2.VideoCapture(0)，C922 是 cv2.VideoCapture(1)

**P7.2 依赖安装**（`pip install -r requirements-voice.txt`）：
- Python 3.14 wheel **现已全面支持**：torch 2.11.0, faster-whisper 1.2.1, ctranslate2 4.7.1, insightface 0.7.3, resemblyzer 0.1.4, opencv-python 4.10.0
- opencv-python-headless 不带 GUI 后端，必须换成 `opencv-python` 才能 cv2.imshow + DirectShow 摄像头枚举
- opencv 4.13 在 Python 3.14/Windows 11 上 MSMF backend 有回归，**降级到 4.10 解决**
- 模型预热：InsightFace buffalo_l (~300MB)、resemblyzer GE2E、faster-whisper base + tiny 全部下载到 `~/.insightface` / `~/.cache/huggingface`

**P7.3 三个真坑**：
1. **Windows 11 全局摄像头权限默认 Deny**：`HKLM\...\CapabilityAccessManager\ConsentStore\webcam Value=Deny` 阻断所有 Python opencv 调用，Settings → 隐私 → 相机 把"相机访问"打开后 OK
2. **sounddevice/PortAudio 在 Windows 上的 hostapi 优先级**：USB 摄像头麦在 MME 报 -9999、DirectSound 报 -9999、WASAPI 报 -9996/-9997，**只有 WDM-KS [28] 能成功打开**。改 `edge/audio_capture.py::_find_input_device` 增加 `_HOSTAPI_PRIORITY = ("Windows WDM-KS", "Windows WASAPI", "Windows DirectSound", "MME")`，按顺序匹配同名设备
3. **resemblyzer 0.1.4 的 `preprocess_wav` 不接受 BytesIO**：只吃 `str/Path/np.ndarray`。改 `edge/voiceprint.py::_load_wav` 用 stdlib `wave` 模块自己解码 WAV bytes 成 float32 ndarray 再传

**新增 / 改动**：
- `scripts/check_devices.py`（新建）：列摄像头 + 音频设备 + 测试拍照 + 测试蜂鸣，UTF-8 stdout，多 hostapi/backend 探测
- `scripts/enroll_owner.py`：加 `--camera-index` 参数（默认 0），加多 backend 自动 fallback（MSMF → DSHOW → ANY），改 import 提示从 headless 到完整版
- `edge/audio_capture.py`：完全重写 `_find_input_device` 走 hostapi 优先级；新增 `_candidate_input_specs()` 候选列表（device × sr × channels）；保留 `_name_overlap()` 助手
- `edge/voiceprint.py::_load_wav`：用 wave 模块手动解码替代 BytesIO 直传

**验证**：
- 录音通了：WDM-KS [28] '麦克风 1 (C922 Pro Stream Webcam)' sr=32000 ch=1，0.5s 录到 8000 帧（重采样到 16k）peak=0.04
- TTS 通了：edge-tts XiaoxiaoNeural 合成 23KB MP3 → miniaudio 解码 → sounddevice 经 Philips USB 音箱播放，听到中文
- 注册产物：`data/enrollments/faces/owner.npy` + `data/enrollments/voices/owner.npy` 都存在

**下一 session 入口**：
```powershell
$env:PYTHONIOENCODING='utf-8'
python main.py --persona assistant --voice
# 呼"小安" → 应触发 wake → 录音 → STT → LLM → TTS → 播放回应
```

预期遇到的问题（按可能性排序）：
1. wake word 检测可能漏触发（sliding window 2s，环境噪声偏高时需调阈值）
2. 蓝牙音箱延迟感人（如要换默认输出走蓝牙：Settings → 系统 → 声音 → 输出）
3. STT 可能慢（Whisper base CPU 推理 ~1s/句），先跑通再考虑换 tiny
4. Tracer 打点要在 voice loop 各阶段加 `time.perf_counter()` 便于 P7.5 测延迟

### 2026-05-10: 启动 P7–P12，写完整愿景完成计划

**背景**：用户硬件到位（蓝牙音箱 + USB 网络摄像头自带麦），要求"确保所有需求都已实现，完成所有愿景"。审视 plan.md §0 愿景对比当前代码，整理出 6 个仍未达成的差距：实机端到端未跑过、主动事件无人消费、情绪感知是存根、流式管道是 batched 形态、多人格共用一个声音、声纹门禁硬编码 False。

**新建**：`docs/vision-completion-plan.md` —— P7–P12 详细实施计划，含：
- 现状盘点（已完成 / 已写代码未验证 / 真存根三类清单）
- 6 个阶段的逐步骤交付物 + 验收清单
- 6 个里程碑（M1 能说话 → M6 愿景达成，估 13 天独立开发节奏）
- 每阶段保持 281 smoke tests 全绿的硬性约束

**改动**：
- `docs/implementation-plan.md`：状态头从 "P0 完成" 更新为 "P1–P6 完成 + P7–P12 启动"，指向 vision-completion-plan.md
- `docs/project-memory.md`：加 Hand-off Snapshot v3，含 P7–P12 目录、未实机验证清单、真存根清单
- `CLAUDE.md`（根 + worktree）：Implementation Status 节加"硬件已到位 / P7–P12 进行中"
- `README.md`：移除"Post-P6 STT/TTS 仍为存根"说明，更新为"代码已写真，硬件验证待跑"
...
**任务**：激活所有硬件存根，实现完整语音对话循环。

**新建文件**：
- `.github/workflows/test.yml`：GitHub Actions CI（Python 3.11 + 3.12，push/PR 触发）
- `backend/tts/edge_tts_client.py`：免费微软 Edge TTS 客户端（`zh-CN-XiaoxiaoNeural`，无 API key）
- `edge/audio_capture.py`：sounddevice 麦克风采集（固定时长 / VAD 静音检测 / 连续流）
- `requirements-voice.txt`：语音硬件依赖单独文件

**修改文件**：
- `backend/streaming/pipeline.py`：`_stt_stage` 真实 Whisper 实现 + `_transcribe()` 工具函数；faster-whisper 缺失时优雅降级
- `backend/tts/__init__.py`：`play_audio_mp3()` + `play_audio_streaming()`（miniaudio 解码 + sounddevice 播放，OS fallback）
- `backend/tts/cosyvoice_client.py`：fallback 链 self_hosted → dashscope → edge-tts → dummy
- `edge/wakeword.py`：Whisper tiny 2s 滑窗关键词检测，`_keyword_in_text()` 含中文别名，ImportError 时静默降级
- `edge/face_gate.py`：insightface buffalo_l 真实实现（load/verify/enroll），ImportError 时 stub 模式
- `edge/voiceprint.py`：resemblyzer GE2E 真实实现（load/enroll/verify），get_voice_activity() 能量 VAD
- `scripts/enroll_owner.py`：cv2 摄像头 5 张注册 + sounddevice 3 段录音，调用真实 FaceGate + VoicePrintGate
- `main.py`：新增 `--voice` flag，`_voice_loop()` 全语音循环，`_check_voice_deps()` 缺包友好提示

**测试**：+24 新 smoke tests（TestEdgeTTSClient×4, TestWhisperSTT×4, TestAudioCapture×3, TestWakeWordReal×4, TestFaceGateReal×5, TestVoicePrintReal×4），全部 mock 硬件依赖。281/281 全绿，12.5s。

**设计决策**：
- **Whisper 关键词检测代替 openwakeword**：openwakeword 无中文预训练模型，Whisper tiny 滑窗开箱即用
- **edge-tts 代替 CosyVoice 作主 fallback**：免费、无服务器、中文质量高，CosyVoice 留作零样本克隆（需单独部署）
- **resemblyzer 代替 speechbrain**：17MB vs 400MB+，API 更简单，余弦相似度足够
- **硬件 ImportError → 优雅降级**：所有 edge 模块在无相应库时保持 stub 行为，CI/文字模式不受影响

### 2026-04-30: 硬件接入测试 + 文档全面同步

**任务**：接上 USB 摄像头（Logitech C922 Pro Stream Webcam，自带麦克风）和 USB 音箱（Philips SPA3809），运行测试，更新所有文档。

**验证结果**：
- **257/257 smoke tests 全绿**，耗时 10.82s（Python 3.14，langchain-core Pydantic v1 警告无害）
- USB 摄像头 C922：Windows PnP 识别，Status OK（Image 类设备）
- C922 内置麦克风：Windows AudioEndpoint 识别，Status OK（输入设备）
- USB 音箱 Philips SPA3809：Windows AudioEndpoint 识别，Status OK（输出设备）
- 内置摄像头 ACER HD User Facing：Status OK（index 0；USB C922 在 Image 类下）
- 所有 secrets 已配置：`llm_keys.env`（AIHUBMIX + BOCHA key）、`accounts.env`（Bilibili 4 cookie + NCM MUSIC_U + __csrf）、`bilibili_credential.json`、`pyncm_credential.json`

**文档改动**：
- `CLAUDE.md`：加 `# CLAUDE.md` 前缀、新增 Implementation Status 节（标注三处存根文件路径）、补充单测类命令 + `--no-tools` + `verify_mcp_servers.py`、修正 Session Rule 3（P1-P6 已完成，改指 `plan.md §3–4`）
- `docs/implementation-plan.md`：状态头从 "P0 完成（185 tests）" 更新为 "P1–P6 全部完成（257 tests，2026-04-30）"，标注为历史文档
- `README.md`：数据流说明后增加 Post-P6 现状提示（CLI 可用，STT/TTS/wake/face/voiceprint 仍为存根，外设已识别）
- `scripts/check_hardware.py`：新增，一次性硬件检测脚本（opencv/sounddevice 有则用，否则 PowerShell PnP fallback）

**项目当前可运行状态**：
```bash
python main.py --persona assistant   # 文字对话，工具调用正常
```
bilibili + pyncm + bocha + memory 四组工具全部加载，speaker_verified=False 模式下约 13/26 工具可见。

**下一步**：
1. 安装 `faster-whisper` + `sounddevice`，替换 `backend/streaming/pipeline.py::_stt_stage` 存根
2. 接入 `edge-tts` 或 DashScope 真实 TTS，替换 `backend/tts/cosyvoice_client.py` 占位
3. 安装 `insightface`，激活 `edge/face_gate.py` 人脸识别路径
